...
import { mapboxGeocodeReverse } from "@/config/mapbox";
...
const StoreInfoTab = ({ restaurant, userId }: StoreInfoTabProps) => {
  ...
  const reverseGeocode = useCallback(async (lat: number, lng: number) => {
    try {
      ...
      console.log("[StoreInfoTab:reverseGeocode]", result);
      if (result.formattedAddress) {
        setForm(f => ({ ...f, address: result.formattedAddress }));
      }
    } catch (err: any) {
      console.error("[StoreInfoTab:reverseGeocode:erro]", err);
    }
  }, []);
